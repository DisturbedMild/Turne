...
function RegisterPicker(scopeSelector) {
    ...
    divDatePicker.prepend("<header>Дата вылета — выберите несколько</header>").append("<footer class='duration modal-fix-bottom'>" +
        ...
        "</div>" +
        "<div class='duration-btn-wrap'><input type=\"button\" class=\"btn btn-brand btn-brand--orange jsPickerClose\" value=\"Выбрать\"></div>"+
        "</footer>");
...
$(window).on('resize', function () {
    if($(this).width() >= 768){
        $('.b-datepicker').datepicker( "option", {"numberOfMonths": 2} );
    } else {
        $('.b-datepicker').datepicker( "option", {"numberOfMonths": 1} );
    }
